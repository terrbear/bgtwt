module Umlatte
  def ck_fu(options={})
    ...
    content_tag :div, :id => 'ck_fu', :class => Rails.environment do
      ...
      text += " #{separator} Revision: #{deployed_revision}" if deployed_revision && (options[:revision].nil? || options[:revision])
      text += " #{separator} Deployed: #{deployed_date}" if deployed_date && (options[:date].nil? || options[:date])
